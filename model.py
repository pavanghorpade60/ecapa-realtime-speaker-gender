...
## Modified for binary gender classification
...
# -------------------------------------------------
# Bottle2neck Block (Res2Net-style block)
# -------------------------------------------------
...
class ECAPA_gender(nn.Module, PyTorchModelHubMixin):
    ...
    # IMPORTANT: MelSpectrogram must stay here
    def logtorchfbank(self, x: torch.Tensor) -> torch.Tensor:

        flipped_filter = torch.tensor(
            [-0.97, 1.0], dtype=x.dtype, device=x.device
